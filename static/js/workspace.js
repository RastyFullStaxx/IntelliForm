...
function initWorkspace() {
  ...
  async function drawOverlay(formId, pageNumber, anchor = null) {
    try {
      ...
      const { sx, sy } = pageScaleFactors(pageNumber); // <-- CRITICAL: define sx/sy here

      // Draw background rectangles only if toggleBoxes is ON
...
async function jumpToLabelText(text){
  ...
  if (checkBtn) {
    checkBtn.addEventListener("click", (ev) => {
      ev.stopPropagation(); // prevent the generic toolbar handler from leaving us on a non-text tool
